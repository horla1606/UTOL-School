...
window.UTOL = {
  ...
  /* ── Auth guard: call at top of every dashboard ──
     ...
     Returns the user object or null (+ redirects to login if not authenticated). */
  async requireAuth(role) {
    ...
    /* Update stored role to match this dashboard — no cross-redirect */
    if (role) localStorage.setItem('utol_role', role);
    return user;
  },
